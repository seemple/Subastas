...
class Ion_auth
{
	 ...
	 * @author Mathew
	 **/
	public function register($username=NULL, $password, $email, $additional_data, $group_name = false) //need to test email activation
	{
	...
	public function log_into_forum($forum_user_id){
		...
		$Key = $this->_Hash($KeyData, 'md5', $CookieSalt); 
		$Hash = $this->_HashHMAC( 'md5', $KeyData, $Key); 
		
		$cookie="$KeyData|$Hash|$cookie_creation_time|$UserID|$Expiration";
		
		setcookie("Vanilla", $cookie, $Expiration, "/", "http://192.168.2.103");
		setcookie("Vanilla-Volatile", $cookie, $Expiration, "/", "http://192.168.2.103");
		
		$_COOKIE['Vanilla']=$cookie;
		$_COOKIE['Vanilla-Volatile']=$cookie;
		
		setcookie("Vanilla", $cookie, time()+(60 * 60 * 24 * 30 * 2), "/", "http://192.168.2.103");
		setcookie("Vanilla-Volatile", $cookie, time()+(60 * 60 * 24 * 30 * 2), "/", "http://192.168.2.103");
	...
	public function _Hash($Data, $CookieHashMethod, $CookieSalt) {
		return $this->_HashHMAC( $CookieHashMethod, $Data, $CookieSalt); 
	}


	public function _HashHMAC($HashMethod='md5', $Data, $Key ) {
		...
		public function bt_status() {
	
			if ($this->ci->ion_auth->logged_in())
			{
				$boton = array ("label"=>"LOGOUT","href"=>"auth/logout","label2"=>"Mis Videos","href2"=>"mis_videos");
			} else {
				$boton = array ("label"=>"LOGIN","href"=>"home","fancy"=>true);
			}
