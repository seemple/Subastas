...
*/
	...
	public function register($username, $password, $email, $additional_data = false, $group_name = false)
	{
	    ...
	    if ($this->identity_column == 'email' && $this->email_check($email))
	    {
			...
			$this->session->set_flashdata("mensaje","El email ingresado ya figura registrado.");
			return FALSE;
	    }
	    elseif ($this->identity_column == 'username' && $this->username_check($username))
	    {
			...
			$this->session->set_flashdata("mensaje","El username ingresado ya figura registrado.");
			return FALSE;
	    }
	    ...
	    if (isset ($additional_data)) {
			...
			foreach ($additional_data as $item =>$value) {
				$data[$item] = $value;
			}
	...
	public function login($identity, $password, $remember=FALSE)
	{
	    ...
	    $query = $this->db->select($this->identity_column.', id, password, group_id,telefono,nombre,apellido')
			      ->where($this->identity_column, $identity)
			      ->where('active', 1)
			      ->where('group_id', 3)
			      ->where($this->ion_auth->_extra_where)
		...
		if ($result->password === $password)
		{
		    ...
		    $session_data = array(
					...
					'email'                => $result->email, //kept for backwards compatibility
